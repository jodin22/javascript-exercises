...
const fibonacci = function(whichMember) {
...
// index 3 which is a new value and that will be put into index 4. and the i++ happens so the next round is...
  ...
  for (let i = 0; i <= 30; i++) { // the i is not only the counter but also used to get each index number.
  ...
  // the rest of the fibonacci sequence.
    ...
    console.log(`${nextIndexValue} is pushed to the new array as index ${i+2}`);
...
// use at(index) instead of findIndex or indexOf bc at(index) only needs the index number. the other two need the element that will
